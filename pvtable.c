...
const int PvSize = 0x100000 * 2;/*2 MB is the pvSize */ 

int GetPvLine(const int depth, S_BOARD *pos) {/*Function to get the pv line */
...
void ClearHashTable(S_HASHTABLE *table) {/* Function to clear the hash table */
...
void InitHashTable(S_HASHTABLE *table, const int MB) {/* Function to initialize the hash-table */
...
/* Function to store the a entry of move in the hash-table */
	int index = pos->posKey % pos->HashTable->numEntries;
...
/*Function to apply alpha-beta search algorithm to the hash-entry */
...
int ProbePvMove(const S_BOARD *pos) {/* Function to probe for a move */
